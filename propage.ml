...
let france_rea =
  [|15;23;39;45;55;
    ...
    (* 21 avril *)
    5334;5127;4967;4785;4641;4598;4526;4392;4207 (* à verifier *)
  |]
;;
...
let m = 700 ;; (* largeur en kilometres du carré représentant le pays, 
...
let div_dvoisins = 10;; (* facteur de réduction après le confinement *)
...
let ncase = i_f (sqrt (f_i n)) + 1;;
let cote = f_i m /. f_i ncase;;
let mat = Array.make_matrix ncase ncase 0;;

let dvoisins = ref (i_f (sqrt (f_i voisins) /. 2.));;

(* choisit un voisin au hasard avec distance 1 < dvoisins 
   rend ses coordonnées *)
...
let lcontacts = Array.make (10*million) (0,0);;
pr "lcontacts créée\n";;
let ncontacts = ref 0;;
...
let erreur_hist hist jconf =
  ...
  with _ -> (0.,jdebut)
;;
(* erreur sur les différences d'un jour au suivant *)
let erreur_hist_diff hist jconf =
  let jdebut = jconf - !debut_confinement in
  try
    let s = ref 0. in
    let compte = ref 0 in
    let diffrmax = ref 0.0000001 in
    for j = jdebut + 1 to jdebut + Array.length !pays_morts - 1   do
      let morts,_,_,_ = hist.(j) in
      let morts1,_,_,_ = hist.(j-1) in
      let jp = !debut_confinement + j - jconf in
      if !pays_morts.(jp) > 100
      then (compte := !compte + 1;
            let diffr = f_i (abs (!pays_morts.(jp) - !pays_morts.(jp-1))) in
            if diffr > !diffrmax then diffrmax := diffr;
            let diffs = f_i (abs (morts - morts1)) in
            s := !s +. (diffs -. diffr)**2.);
    done;
    let e = sqrt (!s /. f_i !compte) in
    (* erreur rapportée à la plus grande différence *)
    (100. *. e /. !diffrmax, jdebut)
  with _ -> (0.,jdebut)
;;
  ...
  try(
    ...
    while !j <= !k do
      ...
      then (
        ...
        if !jconf = -1 then (pr "%d(m:%d,i:%d), " !j morts nencontacts; flush stdout);
        if !jconf <> -1
        then (let jp = !debut_confinement + !j - !jconf in
              if jp > !debut_confinement + 4
                      (* pas bon, ca autorise des erreurs apres le confinement trop grandes 
                         et ainsi des mortalités > 12, ce qui est bizarre :
                        (Array.length !pays_morts - !debut_confinement) / 4 *)
                 && jp < Array.length !pays_morts
...
let dir = !pays ^ "_jour_" ^ string_of_int jourfr;;
let _ = Sys.command ("mkdir " ^ dir)
                    
let fichier_resultats =
  dir ^ "/" ^ (Printf.sprintf "_resultats_%s_jour_%d_%s.csv" !pays jourfr !methode);;
pr "%s\n" fichier_resultats;;
try
...
let fichier_resultats2 =
  dir ^ "/" ^ (Printf.sprintf "_resultats2_%s_jour_%d_%s.csv" !pays jourfr !methode);;
pr "%s\n" fichier_resultats2;;
try
...
let previsions () =
  ...
  let hist,jconf,ncontacts = jour 500 (* 150 *) in
  ...
  then (pr "================================ trop ou pas assez de morts\n";
        ...
        let file = Printf.sprintf "%s/jour_%d_err_%.4f_R0_%.2f_dR0_%.1f_R01_%.2f_pvoy_%.2f_dpvoy_%.0f_debi_%d_duri_%d_mor_%.5f_dc_%d_nc_%d_lm_%d.py" dir
        ...
        if err < !erreur_min || err < 0.02
        then (erreur_min := err;
              let _ = Sys.command ("python3 cree_pdf.py " ^ file) in
              let _ = Sys.command ("python3 prevision.py " ^ dir ^ " 20 5"
                                   (* ^ (string_of_int (i_f !erreur_max)) *) ) in
...
let set_params p =
  ...
  div_vc_avant_confinement := p.(1);
  vc_confinement := p.(2);
  proba_voyageur := p.(3);
  div_proba_voyageur := p.(4);
  debut_infectant := i_f p.(5);
  duree_infectant := i_f p.(6);
  mortalite := p.(7);
;;
(***********************************************************************)
...
(*
          ...
          arret_au_present := true;
 *)
  (* exploration aléatoire 27 avril *)
(*
  ...
  debut_infectant := hasard_int_intervalle 1 9;
  duree_infectant := hasard_int_intervalle 3 21;
  mortalite := hasard_float_intervalle 0.0001 0.15;
  erreur_max := 20.;
  arret_au_present := true;
 *)
  (* on affine avec les intervalles de confiance *)
(*
  voisins_contamines := hasard_float_intervalle 6.8 9.3;
  div_vc_avant_confinement := hasard_float_intervalle 2.3 4.2;
  vc_confinement := hasard_float_intervalle 0.74 1.07;
  proba_voyageur := hasard_float_intervalle 0.06 0.15;
  div_proba_voyageur := hasard_float_intervalle 21. 80.;
  debut_infectant := hasard_int_intervalle 2 5;
  duree_infectant := hasard_int_intervalle 6 15;
  mortalite := hasard_float_intervalle 0.013 0.046;
  erreur_max := 20.;
  arret_au_present := true;
 *)
  (* avec les paramètres moyens *)

  voisins_contamines := 8.8;
  div_vc_avant_confinement := 3.9;
  vc_confinement := 0.96;
  proba_voyageur := 0.09;
  div_proba_voyageur := 50.;
  debut_infectant := 4;
  duree_infectant := 10;
  mortalite := 0.023;
  erreur_max := 20.;
  ...
  pr "temps par prévisions: %.4f\n" ((Sys.time () -. temps_depart) /. f_i !nprevisions);
  ...
  pr "--------------------------------------------------------------\n";
  info_params ();
  let err,ect = previsions_n 1 (* 20 *) in
  if err >= 0.
