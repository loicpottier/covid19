...
def convert(s):
    ...
    m2 = re.compile('_dv_(?P<dv>[0-9]+)'
                   + '_ddv_(?P<ddv>[0-9]+)').search(s)
...
#convert('jour_58_err_1.5632_R0_3.40_dR0_2.2_R01_0.89_pvoy_0.11_dpvoy_13_debi_2_duri_4_mor_0.02992_dc_0_nc_620969_lm_16002_dv_11_ddv_2.py')
...
def trace_tout():
    ...
    plt.savefig(pays + '/' + '_tout_jour_' + str(jour) + '.png')
    #plt.show(False)
...
dv = 11
ddv = 12
nom = ['err','R0','dR0','R01','pvoy','dpvoy','debi','duri','mor','nc','lm','dv','ddv']
...
def joli(param):
    err,R0,dR0,R01,pvoy,dpvoy,debi,duri,mor,nc,lm,dv,ddv = param
    return("[err %.2f, R0 %.1f, dR0 %.1f, R01 %.2f, pvoy %.2f, dpvoy %.0f, debi %d, duri %d, mor %.4f, nc %d, lm %d, dv %d, ddv %d]" % (err,R0,dR0,R01,pvoy,dpvoy,int(debi),int(duri),mor,int(nc),int(lm),int(dv),int(ddv)))
...
for p,pn in enumerate(nom):
    if pn in ['R0','mor','R01','debi','duri','dR0','pvoy','dpvoy','dv','ddv']:
        print(p,pn)
        lx = []
        ly = []
        ly1 = []
        for k in range(100):
            ...
            if lv != []:
                ...
                if pn == 'dv': npmoy = npmoy / 100
                if pn == 'ddv': npmoy = npmoy / 10
                ly.append(npmoy)
        ly = lissage(ly,5)
        text = pn
        if pn == 'mor': text = 'IFR(%)'
        ...
        if pn == 'dv': text = 'dv/100'
        if pn == 'ddv': text = 'ddv/10'
...
def prv(pn,v):
    ...
    if pn in ['dpvoy','dv','ddv']:return("%.0f" % v)
    return("")

f = open(pays + '/_histogramme.csv','w')
ninterv = 8
#f.write(str(len(datas)) + " sim.  err<" + str(erreur_max) + ";average;cri;limit1 (r21);cri1;limit2 (r22);cri2;common interval;estimation\n")
f.write(str(len(datas)) + " sim.  err<" + str(erreur_max) + ";average;cri\n")
fb = open(pays + '/_estimations.csv','w')
...
for p,pn in enumerate(nom):
    ...
    o1,e1,r21 = r1
    o2,e2,r22 = r2
    a = max([0,m-e,o1-e1,o2-e2])
    b = max(min([m+e,o1+e1,o2+e2]),0)
    ...
    if "" not in lv1:
        ...
        #f.write("%s;%s;%s-%s;%s (%s);%s-%s;%s (%s);%s-%s;%s-%s;%s\n" % (pn1,lv1[0],lv1[1],lv1[2],lv1[3],lv1[4],lv1[5],lv1[6],lv1[7],lv1[8],lv1[9],lv1[10],lv1[11],lv1[12],lv1[13]))
        f.write("%s;%s;%s-%s\n" % (pn1,lv1[0],lv1[1],lv1[2]))
        fb.write("%s\n" % pn1)
        #fb.write("%s\n" % lv1[13])
        fb.write("%s\n" % lv1[0])
